...
typedef struct mg_blake3_context mg_blake3_context_t;

// Basic hash computation
mg_result_t mg_blake3_hash(
    const void* data,
    size_t data_size,
    mg_blake3_hash_t* out_hash
);

// Streaming hash computation
mg_result_t mg_blake3_context_create(
    mg_blake3_context_t** out_context
);

mg_result_t mg_blake3_context_destroy(
    mg_blake3_context_t* context
);

mg_result_t mg_blake3_update(
    mg_blake3_context_t* context,
    const void* data,
    size_t data_size
);

mg_result_t mg_blake3_finalize(
    mg_blake3_context_t* context,
    mg_blake3_hash_t* out_hash
);

// Hash verification
mg_result_t mg_blake3_verify(
    const void* data,
    size_t data_size,
    const mg_blake3_hash_t* expected_hash
);

// Bundle integrity
mg_result_t mg_bundle_compute_integrity_hash(
...
typedef struct {
    mg_blake3_hash_t* hashes;  // Hash values at this level
    size_t count;                    // Number of hashes at this level
} mg_merkle_level_t;

typedef struct {
    mg_merkle_level_t* levels; // Tree levels (leaf to root)
    size_t level_count;              // Number of levels in tree
    size_t chunk_size;               // Size of data chunks at leaves
    mg_blake3_hash_t root_hash; // Root hash of the tree
} mg_merkle_tree_t;

// Merkle tree operations
mg_result_t mg_merkle_tree_create(
    const void* data,
    size_t data_size,
    size_t chunk_size,
    mg_merkle_tree_t** out_tree
);

mg_result_t mg_merkle_tree_destroy(
    mg_merkle_tree_t* tree
);

mg_result_t mg_merkle_tree_verify_chunk(
    const mg_merkle_tree_t* tree,
    size_t chunk_index,
    const void* chunk_data,
    size_t chunk_size
);

mg_result_t mg_merkle_tree_get_proof(
    const mg_merkle_tree_t* tree,
    size_t chunk_index,
    mg_blake3_hash_t** out_proof_hashes,
    size_t* out_proof_length
);

mg_result_t mg_merkle_verify_proof(
    const void* chunk_data,
    size_t chunk_size,
    size_t chunk_index,
    const mg_blake3_hash_t* proof_hashes,
    size_t proof_length,
    const mg_blake3_hash_t* root_hash
);

// Hardware acceleration
typedef enum {
    METAGRAPH_BLAKE3_SOFTWARE,      // Pure software implementation
    METAGRAPH_BLAKE3_SIMD,          // SIMD-accelerated (SSE, AVX, NEON)
    METAGRAPH_BLAKE3_HARDWARE       // Dedicated crypto hardware
} mg_blake3_implementation_t;

typedef struct {
    mg_blake3_implementation_t implementation;
    const char* implementation_name;
    uint32_t performance_factor;    // Relative performance vs software
    bool is_available;             // Whether available on this platform
} mg_blake3_capability_t;

mg_result_t mg_blake3_get_capabilities(
    mg_blake3_capability_t** out_capabilities,
    size_t* out_count
);

mg_result_t mg_blake3_set_implementation(
    mg_blake3_implementation_t implementation
);

// Content deduplication
typedef struct mg_deduplication_context mg_deduplication_context_t;

mg_result_t mg_deduplication_context_create(
    mg_deduplication_context_t** out_context
);

mg_result_t mg_deduplication_context_destroy(
    mg_deduplication_context_t* context
);

mg_result_t mg_deduplication_add_content(
    mg_deduplication_context_t* context,
    const void* data,
    size_t data_size,
...
mg_result_t mg_deduplication_get_stats(
    const mg_deduplication_context_t* context,
...
// Hash utilities
mg_result_t mg_blake3_hash_to_string(
    const mg_blake3_hash_t* hash,
    char* buffer,
    size_t buffer_size
);

mg_result_t mg_blake3_hash_from_string(
    const char* hash_string,
    mg_blake3_hash_t* out_hash
);

mg_result_t mg_blake3_hash_compare(
    const mg_blake3_hash_t* hash1,
    const mg_blake3_hash_t* hash2,
    int* out_result
);
...
typedef struct {
    ...
    mg_blake3_implementation_t active_implementation;
} mg_blake3_stats_t;

mg_result_t mg_blake3_get_stats(mg_blake3_stats_t* out_stats);
mg_result_t mg_blake3_reset_stats(void);
```
...
sequenceDiagram
    ...
    App->>Blake3: mg_blake3_context_create()
    ...
    loop For each data chunk
        ...
        App->>Blake3: mg_blake3_update(chunk_data)
        Blake3->>HW: process_chunk(chunk_data)
        HW->>Blake3: chunk_processed
    end

    App->>Blake3: mg_blake3_finalize()
...
typedef struct {
    ...
    alignas(64) mg_blake3_hash_t level_0[MAX_LEAVES];
    alignas(64) mg_blake3_hash_t level_1[MAX_LEAVES/2];
    alignas(64) mg_blake3_hash_t level_2[MAX_LEAVES/4];
    // ... additional levels
